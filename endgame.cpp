...
int EvalEndgame()
{
	...
	if(ply>2)
	{
	int lost = -LostEnding(side,xside);//more?
	if(lost!=0)
	{
		if(side==0)
		  lost += score[0] - score[1];
		else
			lost += score[1] - score[0];
		if(lost<=10000+ply)
			lost = -10000+ply+50;
		return lost;
	}
	lost = LostEnding(xside,side);
	if(lost!=0)
	{
		if(side==0)
		  lost += score[0] - score[1];
	...
	/*
static int best = 0;
int diff = (score[side] - score[xside]) - (s1-s2);
{
if(diff>best && piece_mat[0] && piece_mat[1])
{
		best=diff;
	printf(" diff %d ",best);
	z();
...
*/
//if(abs(score[side] - score[xside])>9900)
//	z();
	return score[side] - score[xside];
}
...
int PawnEndings(const int s, const int xs)
{
	...
	//bit_pawnattacks[0] = GetHashbp0();
	//bit_pawnattacks[1] = GetHashbp1();
	SetPawnFen();
