...
int RookMoveCount(const int x, const BITBOARD blocking_pawns);
...
int EvalEndgame()
{
	...
	if (piece_mat[xside] == 0)
	{
		...
		if (bit_pieces[side][Q] && (piece_mat[xside] < startmat[xside] || piece_mat[side] > startmat[side])
			&& !(bit_pieces[xside][P] & (mask_ranks[xside][5] | mask_ranks[xside][6])))
		{
			return 9050;
		}
		if (bit_pieces[side][R] && piece_mat[xside] < startmat[xside] && 
			pawn_mat[side] >= pawn_mat[xside] &&
			!(bit_pieces[xside][P] & (mask_ranks[xside][Q] | mask_ranks[xside][5] | mask_ranks[xside][6])))
		{
			return 9001;
		}
	}
...
int DrawnEnding(const int s, const int xs)
{
	...
	if (pawn_mat[s] == PVAL && piece_mat[s] == 0 && pawn_mat[xs] == 0)
	{
		...
		if (total[xs][R] == 1)
		{
			...
			if (difference[pieces[s][K][0]][pawn] == 1 && pieces[s][K][0] != to &&
				difference[pieces[s][K][0]][to] == 1 &&
				difference[pieces[xs][K][0]][to] >= dist + difference[pieces[s][K][0]][to] &&
		...
		pawn_mat[xs] == PVAL && pawn_mat[s] == 0)
	{
		...
		if (mask_passed[xs][pawn] & bit_pieces[s][K] &&
			...
			rank[xs][pawn] < 5 && row[rook] == row[pawnplus[s][pawn]])
			return 1;
		return 0;
	} 
...
int SinglePawnEnding(const int s, const int xs)
{
	...
	{
		//doubled rook pawns
		if (!(~mask_files[0] & bit_pieces[xs][P]) && 
			!(~mask_files[7] & bit_pieces[xs][P]))
		{
			int xking = pieces[xs][K][0];
			int king = pieces[s][K][0];
			if (!(mask_passed[xs][king] & bit_pieces[xs][P]) && 
				!(mask_passed[xs][xking] & bit_pieces[xs][P]))
			{
				if ((b[pawnplus[s][king]] == P ||
					b[pawndouble[s][king]] == P))
				{
...
int PawnEndingScore(const int s, const int xs)
{
	...
	while (b1)
	{
		...
		if ((s == side && !(mask_squarepawn[s][x] & bit_pieces[xs][K]) ||
			(s == xside && !(mask_squarepawn[s][pawnplus[s][x]] & bit_pieces[xs][K]))))
		{
			score += SquarePawn(s, xs, x);
		}
		else if (!(mask_path[s][x] & ~bit_kingmoves[pieces[s][K][0]]) &&
			!(bit_kingmoves[pieces[xs][K][0]] & mask[x] && 
			!(bit_kingmoves[pieces[s][K][0]] & mask[x])))
		{
			score += SquarePawn(s, xs, x);
		}
		else
			if (!(mask_path[s][pieces[s][K][0]] & bit_pawnattacks[xs]) && 
				rank[s][pieces[s][K][0]] >= rank[s][pieces[xs][K][0]] &&
				difference[pieces[s][K][0]][x] == 1 && 
				col[pieces[s][K][0]] != col[x])
			{
				score += SquarePawn(s, xs, x);
			}
			else if (!(mask_rookfiles & mask[x]))
			{
				...
				else if ((pawndouble[s][x] == pieces[s][K][0] || 
					pawndouble[s][x] == pieces[s][K][0] - 1 ||
					pawndouble[s][x] == pieces[s][K][0] + 1) && 
					difference[pieces[xs][K][0]][x] > 1)
				{
					score += SquarePawn(s, xs, x);
				}
				else if (rank[s][x] == 4)
				{
					if (rank[s][pieces[s][K][0]] == 5 && 
						difference[pieces[s][K][0]][x] == 1)
					{
						score += SquarePawn(s, xs, x);
					}
...
int SquarePawn(const int s, const int xs, const int p)
{
	...
	if (path == path2)
	{
		...
		if (mask_ranks[s][7] & bit_pieces[xs][K] && 
			!(bit_between[target][pieces[xs][K][0]] & bit_all))
		{
			return QUEENING - ply;
		}
	}
...
int OpposedPawns(const int s, const int xs)
{
	...
	BITBOARD b1,b2;
	b1 = bit_pieces[1][P] >> 7 & bit_pieces[1][P] >> 8 & bit_pieces[1][P] >> 9;
	b1 &= bit_pieces[0][P] << 9  & bit_pieces[0][P] << 8 & bit_pieces[0][P] << 7;
	b1 &= mask_ranks[s][5];
...
int EvalPawnless()
{
	if (startmat[0] != piece_mat[0] || startmat[1] != piece_mat[1])
	{
		...
		if (result != 0 && SafeKingMoves(side, xside) > 0 && 
			(!(bit_kingmoves[pieces[side][K][0]] & bit_units[xside])))
		{
			return result;
		}
	}
	...
	if (piece_mat[side] > piece_mat[xside])
		...
		score[xside] += kingking[pieces[xside][K][0]][pieces[side][K][0]];
	return score[side] - score[xside];
}
